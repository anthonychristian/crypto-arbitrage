...
func makeRandomList(n int) *SkipList {
	s := NewIntMap()
	base := int(1234567890123)
	for i := 0; i < n; i++ {
		//insert := rand.Int()
		entropy := rand.Intn(56890123)
		insert := entropy*100000 + base + entropy
		s.Set(insert, insert)
	}
	return s
}
...
func LookupBenchmark(b *testing.B, n int) {
	...
	for i := 0; i < b.N; i++ {
		entropy := rand.Intn(56890123)
		probes[i] = entropy*100000 + base + entropy
	}
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		s.Get(probes[i])
