...
func detectArbitrage(pairs []Pair, exchanges orderbook.ExchangeMap) (start float64, end float64, notProfitable bool, err error) {
	...
	for index, pair := range pairs {
		...
		if ob.Empty() {
			return 0, 0, false, errors.New("ob not initialized for" + string(pair.symbol))
		}
		...
		if pair.side == Buy {
			if owned.curr != rightCurr {
				return 0, 0, false, errors.New("wrong starting currency " + string(owned.curr))
			}
	...
	if owned.curr != startCurr {
		return 0, 0, false, errors.New("Cycle incomplete! Started with " + string(startCurr) + " end with " + string(owned.curr))
	}
	...
	if owned.funds > 0 {
		...
		if owned.curr != startCurr {
			return owned.funds, endingFund, false, errors.New("Traceback cycle wrong! Started with " + string(startCurr) + " end with " + string(owned.curr))
		}

		if endingFund <= owned.funds {
			return owned.funds, endingFund, true, errors.New("Not profitable for available qty, starting fund = " + strconv.FormatFloat(owned.funds, 'f', -1, 64) + " ending fund = " + strconv.FormatFloat(endingFund, 'f', -1, 64) + " loss = " + strconv.FormatFloat(endingFund-owned.funds, 'f', -1, 64))
		}
		return owned.funds, endingFund, false, nil
	}
	return owned.funds, endingFund, true, errors.New("Not profitable, end fund: " + strconv.FormatFloat(endingFund, 'f', -1, 64))
}
